...
fetch('https://api.github.com/repos/Lakshyyaa/testingcsv/contents/words.csv')
    ...
    .then(y => {
        ...
        return (decodedContent.saveplit('\n'));
    })
    // .substring(1, words[Math.floor(Math.random() * 1000)].length - 1)
    .then(words => {
        let y = [];
        for (let i = 0; i < 2; i++) {
            y.push(words[Math.floor(Math.random() * 1000)]substring(1, words[Math.floor(Math.random() * 1000)].length - 1))
        }
        return y
    })
    .then(y => {
        // io.to(room).emit('startit', y.join(' '))
        log(y)
    })
...
io.on('connection', (socket) => {
    ...
    socket.on('started', (room) => {
        ...
        fetch('https://api.github.com/repos/Lakshyyaa/testingcsv/contents/words.csv')
            ...
            //     let y = [];
            //     for (let i = 0; i < 2; i++) {
            //         y.push(words[Math.floor(Math.random() * 1000)])
            //     }
            //     return y
            // })
            .then(y => {

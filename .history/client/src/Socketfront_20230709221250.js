// MAKE SURE ALL WORDS ARE SMALL AS SHIFT+LETTER WILL WASTE THE WORDS
    ...
    return (
        <div>
            ...
            <div ref={maindiv}>
                ...
                <button onClick={() => setStartby1(t)} disabled={start}>Start</button>
                {/* <button onClick={() => countdown()}>fake button</button> */}
                <h1>WPM: {wordspm}</h1>
                <h1>Accuracy: {accuracy}%</h1>
            </div>
            {/* <Multiplayer /> */}
            <Multiplayer setWords={setWords} setStart={setStart} countdown={countdown} start={start} timerText={timerText} words={words} accuracy={accuracy} wordspm={wordspm} index={index} />
        </div>
    )
}
